...
RcppExport SEXP oem_fit_dense_tall2(SEXP x_, 
                                   SEXP y_, 
                                   SEXP family_,
                                   SEXP penalty_,
                                   SEXP lambda_,
                                   ...
                                   SEXP opts_)
{
    ...
    // initialize pointers 
    oemBaseS<Eigen::VectorXd> *solver = NULL; // solver doesn't point to anything yet
    ...
    if (family(0) == "gaussian")
    {
        solver = new oemDenseS(X, Y, penalty_factor, alpha, gamma, intercept, standardize, tol);
